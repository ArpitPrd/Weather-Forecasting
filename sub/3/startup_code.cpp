...
network read_network(const string& filename) {
    ...
    for (int i = 0; i < bayesian_network_obj.netSize(); ++i) {
        ...
        for (const string& parents_name : parent_names) {
            auto piterator_for_node = bayesian_network_obj.search_node_const(parents_name);
            if (piterator_for_node->get_name() != parents_name) {
                 cerr << "Error: Parent node " << parents_name << " not found for " << child_name << endl;
                 continue;
            }
            parent_ms[parents_name] = current_m;
            ...
            for(int p_val_idx = 0; p_val_idx < n_p_values; ++p_val_idx) {
                parent_val_indices[parents_name][p_values[p_val_idx]] = p_val_idx;
            }
        ...
        while (getline(block_stream, block_line)) {
            ...
            if (block_line.substr(0, 5) == "table") {
                ...
                for (int parent_index = 0; parent_index < parent_names.size(); ++parent_index) {
                    const string& parents_name = parent_names[parent_index];
                    const string& pval = parent_values[parent_index];
                    if (parent_val_indices.count(parents_name) == 0 || parent_val_indices[parents_name].count(pval) == 0) {
                        parent_val_ok = false;
                        break;
                    }
                    int p_val_idx = parent_val_indices.at(parents_name).at(pval);
                    base_index += (long long)p_val_idx * parent_ms.at(parents_name);
                }
...
void write_network(const network& BayesNet, const string& filename) {
    ...
    for (int i = 0; i < BayesNet.netSize(); ++i) {
        ...
        if (parents.empty()) {
            ...
            for(const auto& parents_name : parents) {
                auto piterator_for_node = BayesNet.search_node_const(parents_name);
                if (piterator_for_node->get_name() == parents_name) { 
                    radices.push_back(piterator_for_node->get_no_of_values());
                    table_size *= piterator_for_node->get_no_of_values();
                } else {
                    cerr << "Error (Write): Parent node " << parents_name << " not found." << endl;
                    outfile.close();
                    return;
                }
            }
...
long long get_parent_idx(const Graph_Node& node, const network& bayesian_network_obj, const map<string, int>& var_name_to_id, const map<int, int>& parent_val_indices) {
    ...
    for(const string& parents_name : node.get_Parents()) {
        int parent_index = var_name_to_id.at(parents_name);
        int values_index = 0;
        if(parent_val_indices.count(parent_index)) {
             values_index = parent_val_indices.at(parent_index);
        }
        
        pci += (long long)values_index * m;
        
        auto piterator_for_node = bayesian_network_obj.getNodeConst(parent_index);
...
vector<float> get_init_log_posterior_dist(const Graph_Node& missing_node, const network& bayesian_network_obj, const vector<string>& records_data, const map<string, int>& var_name_to_idx, const float epsilon) {
    ...
    for (const string& parents_name : parents) {
        int parent_index = var_name_to_idx.at(parents_name);
        auto parent_node_iterator = bayesian_network_obj.getNodeConst(parent_index);
        parent_val_indices[parent_index] = get_value_index(*parent_node_iterator, records_data[parent_index]);
    }
    ...
    for (int k = 0; k < no_values_missing; ++k) { 
        ...
        for (const string& ypname : child_parents) {
            ...
            if (childp_idx == missing_index) {
                ...
                values_index = get_value_index(*child_node_iter, records_data[childp_idx]);
            }
            child_parent_val_indices[childp_idx] = values_index;
        }
...
void handle_random_cpt_init_for_node(Graph_Node* node_iterator, std::mt19937& rng, std::uniform_real_distribution<float>& dist) {
    
    int no_of_values = node_iterator->get_no_of_values();
    const auto& old_cpt = node_iterator->get_CPT(); 
    const auto& cpt_mask = node_iterator->get_CPT_mask();
...
void initialize_cpts_randomly(network& bayesian_network_obj, const map<string, int>& var_name_to_id, std::mt19937& rng) {
    ...
    for (int i = 0; i < bayesian_network_obj.netSize(); ++i) {
        auto node_iterator = bayesian_network_obj.getNode(i);
        handle_random_cpt_init_for_node(&(*node_iterator), rng, dist);
    }
}
...
map<int, vector<float>> get_child_counts_for(const network& bayesian_network_obj, const vector<vector<string>>& records_data, const map<string, int>& var_name_to_idx, float alpha) {
    ...
    for (const auto& record_from_data : records_data) {
        if (record_from_data.size() != bayesian_network_obj.netSize()) continue; 

        for (int i = 0; i < bayesian_network_obj.netSize(); ++i) {
            auto node_iterator = bayesian_network_obj.getNodeConst(i);
            int child_val_index = get_value_index(*node_iterator, record_from_data[i]);
            if (child_val_index == -1) continue; 

            map<int, int> parent_val_indices;
            bool is_the_record_valid = true;
            for (const auto& parents_name : node_iterator->get_Parents()) {
                int parent_index = var_name_to_idx.at(parents_name);
                auto parent_node_iterator = bayesian_network_obj.getNodeConst(parent_index);
                int values_index = get_value_index(*parent_node_iterator, record_from_data[parent_index]);
                if (values_index == -1) { 
                    is_the_record_valid = false;
                    break;
                }
                parent_val_indices[parent_index] = values_index;
            }
            if (!is_the_record_valid) continue;
            
            long long cpt_index = get_cpt_entry_idx(*node_iterator, bayesian_network_obj, var_name_to_idx, child_val_index, parent_val_indices);
...
map<int, vector<float>> get_parent_counts_for(const network& bayesian_network_obj, const vector<vector<string>>& records_data, const map<string, int>& var_name_to_idx, float alpha) {
    ...
    for (int i = 0; i < bayesian_network_obj.netSize(); ++i) {
        auto node_iterator = bayesian_network_obj.getNodeConst(i);
        long long denom_size = node_iterator->get_CPT().size() / node_iterator->get_no_of_values();
        dens[i] = vector<float>(denom_size, node_iterator->get_no_of_values() * alpha);
    }
    ...
    for (const auto& record_from_data : records_data) {
        ...
        for (int i = 0; i < bayesian_network_obj.netSize(); ++i) {
            auto node_iterator = bayesian_network_obj.getNodeConst(i);
            if (get_value_index(*node_iterator, record_from_data[i]) == -1) continue;
            ...
            long long parent_config_index = get_parent_idx(*node_iterator, bayesian_network_obj, var_name_to_idx, parent_val_indices);
...
pair<map<int, int>, bool> update_parent_val_indices_for_count(const Graph_Node& node, const network& bayesian_network_obj, const vector<string>& record_from_data, const map<string, int>& var_name_to_id) {
    ...
    for (const auto& parents_name : parents) {
        int parent_index = var_name_to_id.at(parents_name);
        auto piterator_for_node = bayesian_network_obj.getNodeConst(parent_index);
        int values_index = get_value_index(*piterator_for_node, record_from_data[parent_index]);
        if (values_index == -1) { 
...
void handle_record_for_count(const vector<string>& record_from_data, const network& bayesian_network_obj, const map<string, int>& var_name_to_id, map<int, vector<float>>& nums, map<int, vector<float>>& dens) {
    ...
    for (int i = 0; i < bayesian_network_obj.netSize(); ++i) {
        ...
        int child_val_index = get_value_index(*iterator_for_node, record_from_data[i]);
        if (child_val_index == -1) continue; 
        
        long long pci = get_parent_idx(*iterator_for_node, bayesian_network_obj, var_name_to_id, parent_val_indices);
        long long cpt_index = pci * num_child_values + child_val_index;
...
pair<map<int, vector<float>>, map<int, vector<float>>> get_counts_for(const network& bayesian_network_obj, const vector<vector<string>>& records_data, const map<string, int>& var_name_to_idx, float alpha) {
    ...
    for (const auto& record_from_data : records_data) {
        handle_record_for_count(record_from_data, bayesian_network_obj, var_name_to_idx, nums, dens);
    }
...
pair<map<int, vector<float>>, map<int, vector<float>>> initialize_num_den(const network& bayesian_network_obj) {
    ...
    for (int i = 0; i < n_nodes; ++i) {
        auto node_iterator = bayesian_network_obj.getNodeConst(i);
        int no_of_values = node_iterator->get_no_of_values();
        long long cpt_size = node_iterator->get_CPT().size();
...
void handle_case_of_complete_data(const network& bayesian_network_obj, const vector<string>& records_data, const map<string, int>& var_name_to_idx, map<int, vector<float>>& nums, map<int, vector<float>>& dens)  {
    ...
    for (int i = 0; i < n_nodes; ++i) {
        auto node_iterator = bayesian_network_obj.getNodeConst(i);
        int child_val_index = get_value_index(*node_iterator, records_data[i]);
        if(child_val_index == -1) continue;
        
        map<int, int> parent_val_indices;
        for(const string& parents_name : node_iterator->get_Parents()) {
            int parent_index = var_name_to_idx.at(parents_name);
            auto parent_node_iterator = bayesian_network_obj.getNodeConst(parent_index);
            parent_val_indices[parent_index] = get_value_index(*parent_node_iterator, records_data[parent_index]);
        }

        long long pci = get_parent_idx(*node_iterator, bayesian_network_obj, var_name_to_idx, parent_val_indices);
        long long num_index = get_cpt_entry_idx(*node_iterator, bayesian_network_obj, var_name_to_idx, child_val_index, parent_val_indices);
...
void handle_not_missing_node_not_child_of_missing_node(int node_idx, const network& bayesian_network_obj, const vector<string>& records_data, const map<string, int>& var_name_to_idx, map<int, vector<float>>& nums, map<int, vector<float>>& dens) {
    auto node_iterator = bayesian_network_obj.getNodeConst(node_idx);
    int child_val_index = get_value_index(*node_iterator, records_data[node_idx]);
    if (child_val_index == -1) return;
...
void handle_not_missing_node(int node_idx, int missing_index, const network& bayesian_network_obj, const vector<string>& records_data, const vector<float>& posterior_dist, const map<string, int>& var_name_to_idx, map<int, vector<float>>& nums, map<int, vector<float>>& dens) {
    auto node_iterator = bayesian_network_obj.getNodeConst(node_idx);
    const auto& parents = node_iterator->get_Parents();
    int no_of_values = node_iterator->get_no_of_values();
    int xm_no_of_values = posterior_dist.size();

    int child_val_idx = get_value_index(*node_iterator, records_data[node_idx]);
    if (child_val_idx == -1) return;
    ...
    for (int k = 0; k < xm_no_of_values; ++k) {
        ...
        for(const string& parents_name : parents) {
            int parent_index = var_name_to_idx.at(parents_name);
            if (parent_index == missing_index) {
                parent_val_indices[parent_index] = k;
            } 
            else {
                auto parent_node_iterator = bayesian_network_obj.getNodeConst(parent_index);
                parent_val_indices[parent_index] = get_value_index(*parent_node_iterator, records_data[parent_index]);
...
void handle_case_of_incomplete_data(const network& bayesian_network_obj, const vector<string>& records_data, int missing_index, const map<string, int>& var_name_to_idx, map<int, vector<float>>& nums, map<int, vector<float>>& dens) {
    ...
    for (int i = 0; i < n_nodes; ++i) {
        ...
        for (const auto& parents_name : parents) {
            if (var_name_to_idx.at(parents_name) == missing_index) {
